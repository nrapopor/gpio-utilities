...
5.	The values are as follows 
   ...
   | :---: | :---:  | :--- 
   | id | bbb-name  | * The name of the Beaglebone black. if you did not name it before, add the proper entry in your ``C:\Windows\system32\drivers\etc\hosts file`` hosts file >> ``bbb-name 192.168.1.145`` obviously using your own name and the actuall BB Black ip address
   | username | ubuntu/debian/pi | the name of the user id on the BBB
...
7.	Install java JDK (yes JDK not JRE). Please use the latest java (which at the time of this writing    
    ...
    - Create a link to the ``C:\Program Files\Java`` from a path that does     
      ...
      ``Right-Click`` on the ``Cmd.exe`` link and click *Run As Administrator*  for example: 
>
...
This project was created in early 2016 and all the references to maven and Java will probably get old    
very quickly 
